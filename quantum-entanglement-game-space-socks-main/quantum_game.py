...
def print_text1(text, x, y, color=(255, 255, 255)):
    ...
    # If x and y are not provided, calculate the center of the screen
    
    x = (screen.get_width() - text_rect.width) // 2
    
    y = (screen.get_height() - text_rect.height) // 2
...
def blink_screen(duration, blink_speed):
    start_time = time.time()
    while time.time() - start_time < duration:
        # Fill screen with white for the "blink" effect
        screen.fill((0,0,0))
        pygame.display.flip()
        time.sleep(blink_speed)

        # Fill screen back with the original background
        screen.blit(back, (0, 0))
        pygame.display.flip()
        time.sleep(blink_speed)
...
def exit_button_clicked():
    ...
    # Create a rect for the exit button
...
def load(imagename, img_width, img_height):
    ...
    #imagename = os.path.join(base_dir,'\\gate_imgs\\',str(imagename),'.png')
...
globep=os.path.join(base_dir,'pics','globe-pic.png')
globe=pygame.image.load(globep)
globe = pygame.transform.scale(globe, (170, 170))

alicep=os.path.join(base_dir,'pics','alice-pic.png')
alice=pygame.image.load(alicep)
...
sship=pygame.image.load(satellitep)
...
while run:
    ...
    screen_width = screen.get_width()
    screen_height = screen.get_height()
    button_x = screen_width-200-10
    button_y = screen_height - 35
    draw_button("Exit", button_x, button_y, 200,35, (255, 0, 0), (0, 0, 0), border_radius=20)
    #draw_button(text, x, y, width, height, color, text_color, border_radius):
    ...
    if(success==1 and run_button_flag==1):
        ...
        blink_screen(0.5, 0.1)
        print_text1("Circuit Building Unsuccessful, Try Again :)", 40, 590, (255, 0, 0))  # Display the text
